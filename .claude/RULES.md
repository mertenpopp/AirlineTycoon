...
- This gives the room ID from an action ID: SLONG getRoomFromAction(SLONG PlayerNum, SLONG actionId)
...
- Only call GameMechanic::endStrike if item horse shoe has been given to the drunk guy
...
They are all performed in the bank.

bool GameMechanic::takeOutCredit(qPlayer, amountToRaise):
Go to the bank room and raise money. Call qPlayer.CalcCreditLimit() to find out how much can be raised.
Total amount of money owed is stored in qPlayer.Credit. Note that this is a loan an interest has to be paid.
Recommended action ID: ACTION_RAISEMONEY

bool GameMechanic::payBackCredit(qPlayer, amountToPayBack):
Go to the bank room and pay back your loans. Total amount owed is stored in qPlayer.Credit.
Recommended action ID: ACTION_DROPMONEY

bool GameMechanic::emitStock(PLAYER &qPlayer, SLONG neueAktien, SLONG mode):
Issue new shares. Good way to make more money, however, reduces stock price and more float means competitors could take you over.
...
Set the dividend for the airline’s stock. Higher divident costs more money but improves stock price. Note that increases in divident have a delay before they take effect.
...
Agent / booking / freight actions
---------------------------------

These actions send the bot to the special offices where flight jobs can be taken.

bool GameMechanic::takeLastMinuteJob(PLAYER &qPlayer, SLONG jobId, SLONG &outObjectId):
Pick last-minute job. These can go from any city to any other city and typically need to be completed either today or tomorrow. Plane must have required number of seats at least. Plane needs to be able to travel the required distance. There is a premium if completed on time, otherwise, a fine has to be paid. Some jobs have a fine of zero.
Recommended action ID: ACTION_CHECKAGENT1

bool GameMechanic::takeFlightJob(PLAYER &qPlayer, SLONG jobId, SLONG &outObjectId):
Take regular flight job. These will connect the home airport with any other city. Plane must have required number of seats at least. Plane needs to be able to travel the required distance. There is a premium if completed on time, otherwise, a fine has to be paid. Some jobs have a fine of zero.
Recommended action ID: ACTION_CHECKAGENT2

bool GameMechanic::takeFreightJob(PLAYER &qPlayer, SLONG jobId, SLONG &outObjectId):
Take freight job. These can go from any city to any other city. Total freight volume can be transported via multiple trips and/or planes. Plane needs to be able to travel the required distance. There is a premium if total freight volume was transported on time, otherwise, a fine has to be paid. Some jobs have a fine of zero.
Recommended action ID: ACTION_CHECKAGENT3

bool GameMechanic::takeInternationalFlightJob(PLAYER &qPlayer, SLONG cityId, SLONG jobId, SLONG &outObjectId):
Office-side international call action. You can call any number of your international offices to take passenger flight jobs. They all either start or land in the city you are calling. Otherwise, the same rules as for the jobs picked up by ACTION_CHECKAGENT2 apply. Use the following function to check if a specific city can be called: bool GameMechanic::canCallInternational(PLAYER &qPlayer, SLONG cityId)
Recommended action ID: ACTION_CALL_INTERNATIONAL

bool GameMechanic::takeInternationalFreightJob(PLAYER &qPlayer, SLONG cityId, SLONG jobId, SLONG &outObjectId):
Office-side international call action. You can call any number of your international offices to take freight flight jobs. They all either start or land in the city you are calling. Otherwise, the same rules as for the jobs picked up by ACTION_CHECKAGENT3 apply. Use the following function to check if a specific city can be called: bool GameMechanic::canCallInternational(PLAYER &qPlayer, SLONG cityId)
Recommended action ID: ACTION_CALL_INTERNATIONAL
...
bool GameMechanic::clearFlightPlan(PLAYER &qPlayer, SLONG planeId)
